...
class PrintProjectQ(UnrollerBackend):
    ...
    def version(self, version):
        ...
        self._print_code("from projectq.ops import H, Rz, Ry, CNOT, Measure")
...
class ProjectQExecutor(object):
    ...
    @classmethod
    def generator(cls, filename=None, qasm_text=None):
        """
        generate projecq code from qasm
        """
        ast = Qasm(filename=filename, data=qasm_text).parse()
        q_file = open("backends/workspace/ProjectQ/benchmark.py", 'w')

        printq = PrintProjectQ(q_file)
        unr = unroll.Unroller(ast, printq)
        unr.execute()
        printq.print_footer()
    ...
    @classmethod
    def run_simulation(cls, filename=None, qasm_text=None):
        """
        run simulation on ProjectQ
        """
        cls.generator(filename=filename, qasm_text=qasm_text)
